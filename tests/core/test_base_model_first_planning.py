...
def test_cognitive_profile_defaults_to_base_model_first_and_classic_remains_configurable() -> None:
    assert (
        GPDProjectConfig().cognitive_profile,
        GPDProjectConfig(cognitive_profile="classic").cognitive_profile,
        "cognitive_profile" in supported_config_keys(),
    ) == (CognitiveProfile.BASE_MODEL_FIRST, CognitiveProfile.CLASSIC, True)
